...
le SAFE du jour séries, le combiné grosse cote séries, et le PROCHAIN JOUR
(pour que la page ne soit jamais vide tard le soir, quand tous les matchs
du jour sont déjà commencés). MÊMES règles mesurées que l'onglet « Buts
d'affilée » du robot (static/index.html, tabSeries ; seuils :
docs/SPEC-BUTS-AFFILEE.md §8.6) :
...
Source des matchs : data/calendrier.json (multi-sources, reconstruit à
chaque run CI ; exclut les matchs déjà commencés — voulu : on ne « prédit »
pas un match joué). Information chiffrée uniquement : aucun marché
bookmaker dans nos sources.
"""
import datetime
import json
import math
...
def _source():
    """Matchs à venir : calendrier.json (frais) sinon fixtures du modèle."""
    fx = DB.get("fixtures", [])
...
def rows_of(fx, jour):
    out = [r for r in (_ligne(f) for f in fx if f.get("date") == jour) if r]
    out.sort(key=lambda m: (-m["s2"], m["home"], m["away"]))
    return out


def safe_build(cand, elig):
    safe, ligues = [], set()
...
def construit():
    ...
    for m in elig:
        opts = []
        if m["s2"] >= 0.75:
            opts.append((m["s2"], "2+ match", "79 % mesuré (n=115)"))
        if m["s2d"] >= 0.70:
            opts.append((m["s2d"], "2+ " + m["home"], "76 % mesuré (n=82)"))
        if opts:
            cand.append((max(opts), m))
    cand.sort(key=lambda c: (-c[0][0], c[1]["home"]))
    safe, grosse = safe_build(cand, elig)
    # prochain jour avec des matchs (page jamais vide tard le soir)
    prochain = None
    dates = sorted({f.get("date") for f in fx if (f.get("date") or "") > auj})
    if dates:
        pm = rows_of(fx, dates[0])
        prochain = {"date": dates[0], "matchs": pm[:10]}
    return {
        "genere_le": datetime.datetime.now().isoformat(timespec="minutes"),
        "jour": auj,
        "matchs": ms,
        "prochain_jour": prochain,
        "safe": safe,
        "grosse_cote": grosse,
...
if __name__ == "__main__":
    ...
    pj = out["prochain_jour"]
    print(f"series_jour.json : {len(out['matchs'])} matchs du jour, "
          f"safe {len(out['safe'])} jambe(s), grosse cote "
          f"{len(out['grosse_cote']['legs'])} jambe(s), prochain jour : "
          f"{pj['date'] if pj else '—'} ({len(pj['matchs']) if pj else 0} matchs)")
